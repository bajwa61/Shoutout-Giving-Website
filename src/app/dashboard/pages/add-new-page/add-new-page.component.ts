...
export class AddNewPageComponent implements OnInit {
  ...
  mobileBannerBytesTransferred: any = 0;
  mobileBannerTotalBytes: any = 0;
  mobileBannerDownloadURL: Observable < string > ;
  ...
  updatePageData() {
    ...
    switch (this.pageDetail) {
      ...
      case 'home':
        ...
        pageData['bannersMobile'] = this.mobileBanner != null ? this.mobileBanner : [];
  ...
  getHomeData(pageData) {
    ...
    this.mobileBanner = pageData['bannersMobile'];
  }
  ...
  uploadMobileBanner(event) {
    const n = Date.now();
    const file = event.target.files[0];
    const filePath = `pages/mobileBanners/${n}`;
    const fileRef = this.storage.ref(filePath);
    const task = this.storage.upload(`pages/mobileBanners/${n}`, file);
    this.mobileBannerBytesTransferred = 0;
    this.mobileBannerTotalBytes = 0;

    task.snapshotChanges().pipe(
      finalize(() => {
        this.mobileBannerDownloadURL = fileRef.getDownloadURL();
        this.mobileBannerDownloadURL.subscribe(url => {
          if (url) {
            this._notificationsService.success('Banner uploaded successfully...', '', {
              timeOut: 3000,
              clickToClose: true,
            ...
            this.addMobileBanners(url);
          }
        });
      })
    ).subscribe(progressSnapshot => {
      if (progressSnapshot) {
        this.mobileBannerBytesTransferred = progressSnapshot.bytesTransferred;
        this.mobileBannerTotalBytes = progressSnapshot.totalBytes;
      }
    });
  }
  ...
  addMobileBanners(value) {
    if (value) {
      this.mobileBanner.push(value);
    }
  }
